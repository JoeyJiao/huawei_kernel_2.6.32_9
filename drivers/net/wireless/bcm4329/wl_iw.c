...
)
{
	...
	static int  powermode;

	error = dev_wlc_ioctl(dev, WLC_GET_PM, &pm , sizeof(pm));
	printk("%s: pm = %d, return = %d", __FUNCTION__, pm, error);
	...
	p += snprintf(p, MAX_WX_STRING, "powermode = %d", powermode );
...
/*comment this function to avoid compile error*/
#if 0
static int
wl_iw_set_suspend(
struct net_device *dev,
...
)
{
	...
	if (g_onoff == G_WLAN_SET_ON) {
		...
		set_irq_wake(MSM_GPIO_TO_INT(dhd_oob_gpio_num), 0);		
...
)
{
	...
	set_irq_wake(MSM_GPIO_TO_INT(dhd_oob_gpio_num), 1);		
...
int init_ap_profile_from_string(char *param_str, struct ap_profile *ap_cfg)
{
	...
	do {
		...
		if ( ret ) {
			if ( idx < SSID_LEN+5 ){
				/* append to ap_cfg->ssid. */
				ap_cfg->ssid[ssid_len++] = p;
				ap_cfg->ssid[ssid_len] = 0;
				p = str_ptr[0];
				WL_ERROR(("append %c to ssid. %s\n", str_ptr[0], ap_cfg->ssid));
				idx++;
				str_ptr = sep_ptr + idx;
			} else {
				return ret;
	...
	short max_size)
{
	...
	for (i = 0; i < list->count && i < IW_MAX_AP; i++)
	{
		...
		iwe.cmd = SIOCGIWFREQ;
		#if 1 /* fix the error of channel mapping */
...
/*do not support wl_iw_set_suspend command , avoid system can't resume during suspend*/
#if 0
...
#endif
		if (status == WLC_E_STATUS_SUCCESS) {
			WL_ERROR(("%s: ROAMING succeeded with mac %02x:%02x:%02x:%02x:%02x:%02x\n",__FUNCTION__, e->addr.octet[0],\
			    e->addr.octet[1],e->addr.octet[2],e->addr.octet[3],e->addr.octet[4],e->addr.octet[5]));
